...
import com.cloudhubs.trainticket.cancel.entity.*;
...
public class CancelServiceImpl implements CancelService {
    ...
    public Response cancelOrder(String orderId, String loginId, HttpHeaders headers) {
        ...
        Response<CommonOrder> orderResult = getOrderByIdFromOrder(orderId, headers);
        if (orderResult.getStatus() == 1) {
            CancelServiceImpl.LOGGER.info("[cancelOrder][Cancel Order, Order found G|H]");
            CommonOrder order =  orderResult.getData();
            ...
            Response<CommonOrder> orderOtherResult = getOrderByIdFromOrderOther(orderId, headers);
            if (orderOtherResult.getStatus() == 1) {
                CancelServiceImpl.LOGGER.info("[cancelOrder][Cancel Order, Order found Z|K|Other]");

                CommonOrder order =   orderOtherResult.getData();
                if (order.getStatus() == OrderStatus.NOTPAID.getCode()
                        || order.getStatus() == OrderStatus.PAID.getCode() || order.getStatus() == OrderStatus.CHANGE.getCode()) {
...
//                    order.setStatus(OrderStatus.CANCEL.getCode());
    ...
    private String calculateRefund(CommonOrder order) {
        if (order.getStatus() == OrderStatus.NOTPAID.getCode()) {
            return "0.00";
        }
    ...
    private Response cancelFromOrder(CommonOrder order, HttpHeaders headers) {
    ...
    private Response cancelFromOtherOrder(CommonOrder order, HttpHeaders headers) {
    ...
    private Response<CommonOrder> getOrderByIdFromOrder(String orderId, HttpHeaders headers) {
        CancelServiceImpl.LOGGER.info("[getOrderByIdFromOrder][Get Order][orderId: {}]", orderId);
        HttpHeaders newHeaders = getAuthorizationHeadersFrom(headers);
        HttpEntity requestEntity = new HttpEntity(newHeaders);
        String order_service_url = serviceResolver.getServiceUrl("ts-order-service");
        ResponseEntity<Response<CommonOrder>> re = restTemplate.exchange(
                order_service_url + "/api/v1/orderservice/order/" + orderId,
                HttpMethod.GET,
                requestEntity,
                new ParameterizedTypeReference<Response<CommonOrder>>() {
                });
        return re.getBody();
    }

    private Response<CommonOrder> getOrderByIdFromOrderOther(String orderId, HttpHeaders headers) {
